...
export function Toggle({ label, hint, checked, onCheckedChange, className }: ToggleProps) {
  ...
  return (
    <div className={cn("flex items-center justify-between gap-3 rounded-xl border bg-[var(--surface-2)] p-3", className)}>
      <div>
        <label className="text-[13px] font-semibold text-[var(--text-primary)]" htmlFor={id}>
          {label}
        </label>
      ...
      <button
        ...
        className={cn(
          "relative inline-flex h-7 w-12 shrink-0 rounded-full border p-1 transition outline-none focus-visible:ring-4 focus-visible:ring-[var(--accent-surface)]",
